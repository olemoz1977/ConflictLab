...
export function createBlockPlan(formId, blockIndex, rng=Math.random){
...
export class RapidAttempt {
  constructor({sessionId,blockId,blockIndex,formId,attemptNumber,pairs,now=()=>performance.now(),uuid=()=>crypto.randomUUID(),allowNoClear=true,isTraining=false}){
    ...
    this.allowNoClear=allowNoClear; this.isTraining=isTraining; this.startMs=null; this.currentReadyMs=null; this.currentIndex=0;
  ...
  markReady(at=this.now()){
    ...
    return {status:'READY',remainingBudgetMs:Math.max(0,Math.floor(BLOCK_BUDGET_MS-(at-this.startMs)))};
  }
  recordChoice(choice,at=this.now()){
    ...
    const event=this.#event(pair,choice,true,Math.floor(at-this.currentReadyMs),Math.floor(this.currentReadyMs-this.startMs),Math.floor(at-this.startMs));
    this.events.push(event); this.currentIndex++; this.currentReadyMs=null;
  ...
  expire(at=this.now()){
    ...
    for(let i=this.currentIndex;i<3;i++){
      ...
      this.events.push(this.#event(pair,'timeout',presented,null,readyElapsed,BLOCK_BUDGET_MS));
    }
    this.done=true;this.timedOut=true;this.finalElapsedMs=BLOCK_BUDGET_MS;this.currentReadyMs=null;
  ...
  #event(pair,choice,presented,latency,readyElapsed,eventElapsed){
    ...
    return {eventId:this.uuid(),sessionId:this.sessionId,blockId:this.blockId,blockIndex:this.blockIndex,formId:this.formId,blockAttemptId:this.attemptId,
      ...
      assetAId:pair.aId,assetBId:pair.bId,assetAPosition:pair.aPosition,assetBPosition:pair.bPosition,pairPresented:presented,
