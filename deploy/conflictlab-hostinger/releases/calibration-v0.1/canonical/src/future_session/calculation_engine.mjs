...
export function calculateDirectionalBalance({ events, gateDConfig, domain }) {
  ...
  for (const event of primaryEvents) {
    ...
    if (!mapping || mapping.mapping_status !== 'VALIDATED' || mapping.domain !== domain) {
      continue;
    }
    ...
    if (presented && (event.choice === 'A' || event.choice === 'B')) {
      direction = event.choice === 'A'
        ? mapping.asset_a_direction
        : mapping.asset_b_direction;
    ...
    perPair.push({
      ...
      direction,
      gateDStatus: mapping.mapping_status,
      domain: mapping.domain,
    });
  }

  const nDirectionalChoices = nPos + nNeg;
  const coverage = nEligiblePresentations === 0
    ? NOT_ESTIMABLE
    : nDirectionalChoices / nEligiblePresentations;
