...
| Stage F (session comparison) is validated | EXPERIMENTAL | Implemented and lightly tested; no evidence of whether the comparison is meaningful to users | "A between-period comparison feature exists and uses neutral language" | "The comparison reveals a real psychological shift" |
