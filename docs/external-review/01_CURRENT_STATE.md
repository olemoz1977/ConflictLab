...
| Session reflection | After all pairs, person sees a summary and may write one observation or select "I do not notice anything yet" |
...
| Comparison (Stage F) | Appears only after 6 eligible sessions; compares two adjacent 3-session windows; uses neutral, magnitude-based language with no "improved/worsened" framing |
...
All Stage H (export/import) and Stage J0 (data reset) scenarios listed in their respective specifications were executed on a physical phone across multiple browsers (Chrome, Edge, and at least one additional browser used as a "clean" test environment). Stage I (literal choice reflection) button-activation and history-display fixes were also confirmed on a physical device after two rounds of real-device bug reports. Namespace-safety of the data-reset function (Stage J0, scenario E) was confirmed only through code-logic inspection, not a live multi-namespace browser test.
